...
@app.get('/recipes/{recipe_id}', response_model=schemas.RecipeSchemaOut, tags=['recipes'])
async def get_recipe_by_id(recipe_id: int) -> models.Recipe:
    """ Получить детальную информацию о рецепте """
    async with session.begin():
        query = select(models.Recipe).filter_by(id=recipe_id).options(selectinload(models.Recipe.ingredients))
